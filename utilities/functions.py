    ...
    for i in range(0, pCentroidPred):
        if sorted_dist[i][0] == 0.0:
            ...
            common_classes[sorted_dist[i][1]] += ((1 / (i + 1)) *
                                                  ((sorted_dist[len(sorted_dist) - 1][0] - sorted_dist[i][0]) / (
                                                              sorted_dist[len(sorted_dist) - 1][0] - sorted_dist[0][
                                                          0])))
    ...
    for iClass in range(nClassTotal):
        ...
        for jCent in range(len(centWtClass[iClass])):
            if centWtClass[iClass][jCent] > 1:
                temp = list(np.random.multivariate_normal(centClass[iClass][jCent], covaClass[iClass][jCent],
                                                          how_many_per_centroid))
                xTrain.extend(temp)
...
def aff_simple(pBiasType, centWtClass, centStdClass, rAccClass, rAccClass0, pMod):
    ...
    # lowest accuracy
        ...
        for iClass in range(nClassTotal):
            if rAccClass[iClass] == 0:
                temp = 1 / accFix
            else:
                temp = 1 / rAccClass[iClass]
            working[iClass] = temp
...
def SVM_redistrict(xTrainBatch, yTrainBatch, xNew, yNew, xTestTot, yTestTot, prevSplit, redClass, pACSType, pSeed,
                   iInc):
    nClassTotal = len(redClass)
    ...
    if iInc == 0:
        ...
        iF = 0
        if len(prevSplit) > 0:
            lenOld = np.max([np.max(x) for x in prevSplit]) + 1
        else:
            lenOld = 0
