...
cifars = pd.read_excel('env/cifar-labels.xlsx')
cifars = list(cifars.item_fine)
cifars = [x.strip() for x in cifars]
...
cifar_uri =[]
for cifar in cifars:
    temp = obj_to_uri(cifar, uri_check)
    cifar_uri.append(temp)
    
cifar_vec = []
for uri in cifar_uri:
    temp = nb.loc[[uri]]
    temp = temp.values[0]
    cifar_vec.append(temp)

available = block_uri.copy()        
best_block = [] 
best_uri = []

for j in range(len(cifar_vec)):
    v1 = cifar_vec[j]
    uri1 = cifar_uri[j]
    dmin = 10000
    tempBestBlock = ''
...
#trying to get an even split
min_std = len(set(cifars))
min_ix = 0
for i in range(20):
    kmeans = KMeans(n_clusters=4, random_state = i).fit(cifar_vec)
...
kmeans = KMeans(n_clusters=4, random_state = min_ix).fit(cifar_vec)
centroids = kmeans.cluster_centers_                
labs = kmeans.labels_              
                                      
df = pd.DataFrame(zip(cifars, best_block, labs), columns = ['cifar', 'block', 'building'])
df = df.sort_values(by = ['building'])
df = df.reset_index()
df = df.rename(columns={"index": "classNo"})

df.to_excel('env/cifar-mapping.xlsx')
